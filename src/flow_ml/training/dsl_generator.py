"""LoRA fine-tune for the DSL Generator (English -> Flow DSL).

Structurally identical to `spool_interpreter.train_spool` - same SmolLM2 base
model, same ChatML formatting via `tokenizer.apply_chat_template`, same
LoRA config, same Trainer/TrainingArguments. The differences are local:

- Default prompt_spec lives at `datasets/dsl_generator/prompt_spec.json`.
- Each sample's input field is `description` (was `sanitized_spool`).
- The supervised target is `{"dsl": <sample.dsl>}` (was the spool-interp JSON).
- Sanitization is a no-op (no PII in the hand-authored corpus).
"""
...
class DslTrainConfig(BaseModel):
    ...
    @classmethod
    def from_yaml(cls, path: str | Path) -> "DslTrainConfig":
        return cls(**read_yaml(path))


@dataclass
class DslTrainResult:
    out_dir: Path
...
def render_target(sample: dict) -> str:
    """Render the supervised target string `{"dsl": "<dsl text>"}` for a DslSample row."""
    return json.dumps({"dsl": sample["dsl"]}, ensure_ascii=False)
...
def build_chat_example(
    ...
    *,
    ...
    """Tokenize a single sample into input_ids + labels (loss masked on prompt)."""
    user_text = prompt_spec["user_template"].replace(
        "<<USER_DESCRIPTION>>", sample["description"]
    )
...
def train_dsl(
    ...
    *,
    ...
    """Fine-tune the DSL Generator (LoRA) from a ``ModelDefinition``."""
    training_dict = model_def.merged_smoke() if smoke else dict(model_def.training)
    cfg = DslTrainConfig(**training_dict)
    if seed is not None:
        cfg.seed = seed
    set_seed(cfg.seed)
    ...
    console.print(
        f"[cyan]DSL train[/]: model={cfg.model_id} train={len(train_rows)} val={len(val_rows)} lora={cfg.lora.enabled}"
    )
    ...
    collator = DslDataCollator(tokenizer, prompt_spec, max_length=cfg.max_input_tokens)
    train_ds = _ListDataset(train_rows)
    val_ds = _ListDataset(val_rows) if val_rows else None

    # Disable mid-training eval on MPS: the Trainer leaves val-set tensors in
    # unified memory between eval and the next training step, causing a runaway
    # slowdown.  A single post-training eval pass below is sufficient.
    total_steps = (
    ...
    # Post-training eval: run once after weights are frozen, then clear MPS cache.
    eval_metrics: dict = {}
...
def train() -> None:
    console.print(
        "Use flow_ml.training.dsl_generator.train_dsl(config, dataset_dir, out_dir)"
    )
