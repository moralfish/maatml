...
class SpoolTrainConfig(BaseModel):
    ...
    @classmethod
    def from_yaml(cls, path: str | Path) -> "SpoolTrainConfig":
        return cls(**read_yaml(path))


@dataclass
class SpoolTrainResult:
    out_dir: Path
...
def render_target(sample: dict, prompt_spec: dict) -> str:
    """Render the target JSON string for a SpoolSample row, key order from prompt_spec."""
    keymap = {
        "summary": sample["root_cause"],
        "status": sample["status"],
        "returnCode": sample.get("return_code"),
        "rootCause": sample["root_cause"],
        "suggestedFix": sample["suggested_fix"],
        "confidence": 1.0,
    }
    ordered = {k: keymap.get(k) for k in prompt_spec["json_keys_order"]}
...
def build_chat_example(
    ...
    *,
    ...
    user_text = prompt_spec["user_template"].replace("<<SANITIZED_SPOOL>>", sample["sanitized_spool"])
    target = render_target(sample, prompt_spec)
...
class SpoolDataCollator:
    ...
    def __call__(self, batch: list[dict]) -> dict[str, torch.Tensor]:
        examples = [build_chat_example(row, self.prompt_spec, self.tokenizer, max_length=self.max_length) for row in batch]
...
def train_spool(
    ...
    *,
    ...
    """Fine-tune the Spool Interpreter (LoRA) from a ``ModelDefinition``."""
    training_dict = model_def.merged_smoke() if smoke else dict(model_def.training)
    cfg = SpoolTrainConfig(**training_dict)
    if seed is not None:
        cfg.seed = seed
    set_seed(cfg.seed)
    ...
    console.print(
        f"[cyan]Spool train[/]: model={cfg.model_id} train={len(train_rows)} val={len(val_rows)} lora={cfg.lora.enabled}"
    )
    ...
    collator = SpoolDataCollator(tokenizer, prompt_spec, max_length=cfg.max_input_tokens)
...
def train() -> None:
    console.print("Use flow_ml.training.spool_interpreter.train_spool(config, dataset_dir, out_dir)")
