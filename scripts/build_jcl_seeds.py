...
`flow_ml.data.synthetic.jcl_generator` to produce a balanced corpus of
~500 samples across all eight categories (valid + 7 error codes), then
gates every sample through the 6-layer `validate_jcl_result` check
before writing it to `models/jcl-validator/datasets/samples/seed_samples.jsonl`.

No API calls. Reproducible given the seed. Run anytime to regenerate
or extend the corpus.

Usage:
    python scripts/build_jcl_seeds.py                # default 500 samples
    python scripts/build_jcl_seeds.py --target 800   # larger corpus
    python scripts/build_jcl_seeds.py --append       # keep existing seeds, only add new ones
"""
...
from flow_ml.data.schemas import ErrorCategory  # noqa: E402
from flow_ml.data.synthetic.jcl_generator import (  # noqa: E402
    INJECTORS,
    _load_templates,
    _render_template,
    DEFAULT_TEMPLATE_DIR,
)
from flow_ml.utils.io import stable_hash  # noqa: E402
from flow_ml.validation import validate_jcl_result  # noqa: E402
