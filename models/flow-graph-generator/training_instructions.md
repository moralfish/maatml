...
The model output must be a Flow Graph JSON which is based on react-flow `ReactFlowJsonObject`. The canonical type definitions live in flow-studio at `apps/shared-types/src/graph.ts` (`FlowGraphDto`, `FlowNodeDto`, `FlowEdgeDto`). Below is the shape the model must emit.
...
```json
{
  ...
  "nodes": [
    {
      "id": "string",
      "type": "action | ai | cloud_ai | utility",
      "position": { "x": 0, "y": 0 },
      "data": { "label": "string" }
    }
  ],
...
`warnings` is the model's safety / ambiguity surface. It is populated when the request is unsafe, unsupported, or under-specified. Empty (or omitted) when the proposal is fully confident.

Each node must contain:

- `id` (string) — unique within the graph; use lowercase identifiers like `validate-jcl`, `submit-job`.
- `type` (string) — one of `action`, `ai`, `cloud_ai`, `utility`. Other values are rejected.
- `position` (object) — `{ x: number, y: number }`. The model can use simple grid layout (e.g. step k → `{ x: k * 200, y: 100 }`); flow-studio's UI relays out on import.
- `data` (object) — kind-specific payload. Always includes `label` (a short human-readable string). Other fields depend on `type`:
  - **action**: `adapter` (string), `actionId` (string), plus adapter-specific scalar fields like `command`, `args`, `cwd`, `connectionId`.
  - **ai**: `modelId` (string). Optional `thresholdHigh` (number), `thresholdLow` (number), `input` (string).
  - **cloud_ai**: `provider` (string), `modelId` (string), `prompt` (string). Optional `maxTokens` (number), `temperature` (number), `auditContent` (boolean).
  - **utility**: `actionId` (string). Optional `durationMs` (number, when actionId=sleep), `message` and `level` (string, when actionId=log), `name` and `value` (string, when actionId=set-variable).

Each edge must contain:

- `id` (string) — unique. Convention: `e-<source>-<outcome>-<target>`.
- `source` (string) — must reference an existing node id.
- `target` (string) — must reference an existing node id.
- `outcome` (string) — one of `pass`, `fail`, `always`. Optional; defaults to `always` when omitted. `pass` fires when the source node succeeded, `fail` when it failed or was skipped.
- `label` (string, optional) — human-readable edge label.
- `condition` (string, optional) — opaque condition expression interpreted by the executor's routing layer.

The model must return JSON only. It must not return markdown, explanations, comments, or code fences. The full output must parse with `JSON.parse(...)`.

---

## 5. Initial Node Vocabulary

Start with a small controlled vocabulary. The vocabulary is grounded in flow-studio's actual adapter handlers — only `(type, adapter, actionId)` triples that have a real handler are allowed. The full enumeration is committed to `models/flow-graph-generator/datasets/node_contracts.json` and synced from flow-studio's `crates/flow-adapter-*` registry; the lists below are the v1 surface.

Allowed node `type` values (closed enum from `apps/shared-types/src/graph.ts`):
...
`trigger`, `zoweCommand`, and `mriToolkit` exist in the type union but are reserved or specialised; they are not part of the v1 vocabulary the model emits. Adapters reachable via `action + zowe + cli-raw` cover the Zowe surface; the toolkit surface goes through `action + mri-toolkit + prepare`.

Allowed `(type, adapter, actionId)` triples for `action` nodes:

```text
action + shell + run-command
action + shell + git
action + shell + npm
action + shell + pnpm
action + shell + cargo
action + shell + kubectl
action + shell + curl
action + zowe + cli-raw
action + mri-toolkit + prepare
```

Allowed `utility.data.actionId` values:
...
Allowed `ai.data.modelId` values:
...
Allowed `cloud_ai.data.provider` values:
...
Forbidden adapter names (registered in flow-studio as mock placeholders; the model must not emit them in production output):
...
Forbidden operation classes (the model must refuse, returning an empty or partial graph plus a `warnings` entry):
...
These names are illustrative categories — they are not real adapters in flow-studio. The model is trained to refuse the *category*: anything that touches credentials, secrets, raw shell beyond the curated `shell` allowlist, external HTTP posts, or arbitrary file uploads.

Unsupported or forbidden operations must result in an empty or partial graph with `warnings`. The model must never invent new node types, adapter names, or actionIds outside the lists above.
